...
struct Plane : public Shape {
    ...
    HitInfo intersect(const Ray& ray) const override {
        ...
        info.u = info.point.x * 2.0f;  // Чёткая текстура
...
// Перестроить сцену (только объекты, не камеру и не свет)
void rebuildScene() {
    for (auto* s : scene) delete s;
    scene.clear();

    static Texture sphereTex = Texture::loadBMP("sphere.bmp");
    static Texture floorTex = Texture::loadBMP("floor.bmp");

    Vec3 baseSphere = Vec3(0, 1, 0);
    Vec3 t0 = Vec3(2, 0, 0);
    Vec3 t1 = Vec3(3, 0, 0);
    Vec3 t2 = Vec3(2, 1, 0);
    Vec3 t3 = Vec3(2, 0, 1);

    Vec3 rSphere = rotateY(baseSphere, sceneRotation);
    Vec3 r0 = rotateY(t0, sceneRotation);
    Vec3 r1 = rotateY(t1, sceneRotation);
    Vec3 r2 = rotateY(t2, sceneRotation);
    Vec3 r3 = rotateY(t3, sceneRotation);

    scene.push_back(new Sphere(rSphere, 1.0f,
        Material(Color(1, 0, 0), 0.05f, 0.6f, 0.4f, 0.35f, 128, sphereTex.data ? &sphereTex : nullptr), true));

    scene.push_back(new Tetrahedron(r0, r1, r2, r3,
        Material(Color(0, 0, 1), 0.05f, 0.6f, 0.4f, 0.4f, 128, nullptr), true));

    scene.push_back(new Plane(Vec3(0, -1, 0), Vec3(0, 1, 0),
        Material(Color(1, 1, 1), 0.1f, 0.85f, 0.2f, 0.3f, 64, floorTex.data ? &floorTex : nullptr), true));
}
        ...
        threads.emplace_back([startRow, endRow, tanHalfFov, aspect]() {
            for (int y = startRow; y < endRow; y++) {
                for (int x = 0; x < RENDER_WIDTH; x++) {
                    ...
                    finalColor = finalColor * 0.25f; // rblblrlbrlbrlblrbl
...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    // Движение камеры
    if (key == GLFW_KEY_W) { camera.z -= moveStep; changed = true; }
    else if (key == GLFW_KEY_S) { camera.z += moveStep; changed = true; }
    else if (key == GLFW_KEY_A) { camera.x -= moveStep; changed = true; }
    else if (key == GLFW_KEY_D) { camera.x += moveStep; changed = true; }
    else if (key == GLFW_KEY_Q) { camera.y += moveStep; changed = true; }
    else if (key == GLFW_KEY_E) { camera.y -= moveStep; changed = true; }

    // Поворот объектов (экспозиция!)
    else if (key == GLFW_KEY_Z) { sceneRotation -= rotStep; changed = true; }
    else if (key == GLFW_KEY_X) { sceneRotation += rotStep; changed = true; }

    // Свет
    else if (shift) {
        if (key == GLFW_KEY_LEFT) { lightPos.x -= lightStep; changed = true; }
        else if (key == GLFW_KEY_RIGHT) { lightPos.x += lightStep; changed = true; }
        else if (key == GLFW_KEY_UP) { lightPos.y += lightStep; changed = true; }
        else if (key == GLFW_KEY_DOWN) { lightPos.y -= lightStep; changed = true; }
    }
    ...
    if (changed) {
        ...
        renderImage();
        printf("Cam: (%.2f, %.2f, %.2f) | Light: (%.2f, %.2f, %.2f) | Rot: %.2f\n",
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(DISPLAY_WIDTH, DISPLAY_HEIGHT, "Лабораторная №4 — Трассировка лучей", nullptr, nullptr);
    if (!window) { glfwTerminate(); return -1; }

    glfwMakeContextCurrent(window);
    glfwSetKeyCallback(window, keyCallback);

    rebuildScene(); // Инициализация сцены
    ...
    printf("Управление:\n");
    printf("  WASD/QE — движение камеры\n");
    printf("  Z/X — поворот объектов (экспозиция)\n");
    printf("  Shift+стрелки — свет\n");
    printf("  R — рендер, Esc — выход\n");
    renderImage();
