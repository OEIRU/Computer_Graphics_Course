// Лабораторная №4 — Сферы + Тетраэдры + Текстуры + SSAA + 3D-камера
// Вариант 2: Сферы + Тетраэдры + Зеркальная плоскость
// Соответствует методичке: обратная трассировка, тени, отражения, enabled, управление
...
Color traceRay(const Ray& ray, int depth = 0) {
    ...
    if (!closest.hit) return Color(0.05f, 0.05f, 0.1f);
...
int main() {
    ...
    scene.push_back(new Plane(
        ...
        Material(Color(0.9f, 0.9f, 0.9f), 0.05f, 0.5f, 0.3f, 0.5f, 256, floorTex.data ? &floorTex : nullptr),
        true
    ));
